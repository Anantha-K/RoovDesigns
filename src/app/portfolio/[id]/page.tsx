    ...
    const ctx = gsap.context(() => {
      ...
      ScrollTrigger.matchMedia({
        "(min-width: 768px)": function () {
          gsap.utils
            .toArray<HTMLElement>(".editorial-block")
            .forEach((block) => {
              gsap.fromTo(
                block,
    ...
    <div className="min-h-screen bg-[#0f0e11] text-white flex flex-col">
      <main className="flex-grow">
        ...
        >
          ...
          <div className="absolute inset-0 -top-[10%] h-[120%] w-full pointer-events-none">
            <Image
              ref={imageRef}
              src={project.image}
              alt={project.title}
          ...
          >
            <div className="flex-1">
              ...
              <h1 className="font-display text-[clamp(4rem,10vw,12rem)] leading-[0.85] tracking-tighter text-white drop-shadow-2xl">
                {project.title}
              </h1>
            </div>
        ...
        <section
          ref={contentRef}
          className="w-full bg-[#0f0e11] px-6 md:px-12 py-24 md:py-48 relative z-20"
        >
          ...
          {project.editorial && (
            <div className="max-w-5xl mx-auto mb-32 md:mb-48 space-y-16 md:space-y-24 px-6 md:px-0">
              {project.editorial.map((section, idx) => (
                <div
                  key={idx}
                  className="editorial-block grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-16 origin-center will-change-transform"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-center mt-12 md:mt-24">
                <div className="md:col-span-4 md:col-start-2 reveal-text order-2 md:order-1 px-6 md:px-0 text-center md:text-left">
                  <p className="font-display text-2xl md:text-4xl leading-[1.1] text-white/90">
                    &ldquo;Every detail was meticulously crafted to ensure the
                    architecture speaks softly, yet profoundly.&rdquo;
                  </p>
                </div>
